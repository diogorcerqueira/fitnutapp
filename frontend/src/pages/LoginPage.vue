...
async function handleSubmit() {
  ...
  try {
    ...
    error.value = extrairMensagemErro(e) || 'Credenciais inválidas. Tenta novamente.'
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="min-h-screen flex items-center justify-center bg-gray-900">
    <div class="bg-gray-800 rounded-xl p-8 w-full max-w-md shadow-xl">
      <h1 class="text-2xl font-bold text-white mb-6">Entrar</h1>
      ...
      <a
        ...
        class="flex items-center justify-center gap-3 w-full bg-white hover:bg-gray-100 text-gray-800 font-semibold py-2 rounded-lg transition-colors"
      >
      ...
      <p class="mt-4 text-sm text-gray-400 text-center">
        Não tens conta?
        <RouterLink to="/register" class="text-indigo-400 hover:text-indigo-300">Registar</RouterLink>
      </p>
    </div>
  </div>
</template>
